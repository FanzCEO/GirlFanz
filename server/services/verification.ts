...
import { VerifymyService, IDVerificationRequest } from '../compliance/verifymy';
import type { 
  User, 
  KycVerification,
  AuditLog 
...
export interface VerificationSession {
  ...
  status: 'pending' | 'document_uploaded' | 'selfie_captured' | 'processing' | 'completed' | 'failed';
  documentData?: {
    type: string;
...
export interface VerificationResult {
  verified: boolean;
  status: 'verified' | 'rejected' | 'pending' | 'processing' | 'expired' | 'none';
  confidence?: number;
  validUntil?: Date;
  verifiedAt?: Date;
...
class VerificationService {
  private verifymyService: VerifymyService;
  private sessions: Map<string, VerificationSession> = new Map();
  private readonly SESSION_DURATION = 30 * 60 * 1000; // 30 minutes
  private readonly VERIFICATION_VALIDITY_DAYS = 365; // 1 year
  private readonly REVERIFICATION_WARNING_DAYS = 30; // Warn 30 days before expiry
  ...
  async createSession(userId: string, userType: 'creator' | 'co-star' = 'creator'): Promise<VerificationSession> {
  ...
  async submitVerification(
    ...
    try {
      ...
      const verificationRequest: IDVerificationRequest = {
  ...
  async getUserVerificationStatus(userId: string): Promise<VerificationResult> {
    try {
  ...
  async getVerificationResult(sessionId: string): Promise<VerificationResult | null> {
    const session = this.getSession(sessionId);
  ...
  async needsReverification(userId: string): Promise<boolean> {
    ...
    const daysUntilExpiry = Math.floor(
      (status.validUntil.getTime() - Date.now()) / (1000 * 60 * 60 * 24)
    );
  ...
  private async handleVerificationSuccess(userId: string, verificationId: string): Promise<void> {
    // Update user profile
  ...
  private async storeVerificationImage(userId: string, imageBase64: string, type: string): Promise<string> {
  ...
  private groupByDocumentType(verifications: any[]): Record<string, number> {
  ...
  private groupByProvider(verifications: any[]): Record<string, number> {
    const grouped: Record<string, number> = {};
