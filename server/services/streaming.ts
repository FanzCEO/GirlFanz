...
export interface StreamSession {
  ...
  gifts: StreamGift[];
  reactions: Map<string, number>; // reaction type -> count
...
export interface StreamChatMessage {
  ...
  username: string;
  message: string;
  messageType: 'text' | 'emote' | 'announcement' | 'system';
  timestamp: Date;
  isPinned?: boolean;
}
...
export interface StreamHighlight {
  ...
  startTime: number;
  endTime: number;
  type: 'ai_detected' | 'manual' | 'peak_viewers' | 'peak_gifts';
  score: number;
  timestamp: Date;
}
...
export interface StreamAnalytics {
  ...
  viewerRetention: { [time: string]: number };
}

export interface StreamConfiguration {
  title: string;
  description?: string;
  visibility: 'free' | 'subscriber' | 'paid' | 'followers';
  priceInCents?: number;
  coStarIds?: string[];
  requiresVerification?: boolean;
  maxViewers?: number;
  streamQuality?: '360p' | '720p' | '1080p' | '4K';
...
export class StreamingService {
  ...
  async removeCoStar(sessionId: string, userId: string): Promise<void> {
    const session = this.sessions.get(sessionId);
    if (!session) {
      throw new Error('Stream session not found');
    }
  ...
  async removeViewer(sessionId: string, userId: string): Promise<void> {
    const session = this.sessions.get(sessionId);
    if (!session) return;
  ...
  async sendChatMessage(
    ...
    userId: string,
    message: string,
    messageType: 'text' | 'emote' | 'announcement' = 'text'
  ): Promise<void> {
    const session = this.sessions.get(sessionId);
    if (!session) {
      throw new Error('Stream session not found');
  ...
  private createInitialAnalytics(): StreamAnalytics {
    return {
      currentViewers: 0,
      peakViewers: 0,
      totalViewers: 0,
  ...
  private broadcastToSession(session: StreamSession, message: any): void {
  ...
  private async saveHighlights(session: StreamSession): Promise<void> {
  ...
  private async saveAnalytics(session: StreamSession): Promise<void> {
  ...
  private async processRecordingWithAI(session: StreamSession): Promise<void> {
    if (!session.recordingUrl) return;
  ...
  private async shouldModerateMessage(message: string): Promise<boolean> {
  ...
  private getGiftAnimation(giftType: string): string {
  ...
  private addHighlight(
    ...
    data: { type: StreamHighlight['type']; title?: string; score: number }
  ): void {
  ...
  private async detectHighlights(): Promise<void> {
    ...
    for (const [sessionId, session] of this.sessions) {
      ...
      const recentMessages = session.chatMessages.filter(
  ...
  private startAnalyticsTracking(session: StreamSession): void {
