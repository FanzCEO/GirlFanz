...
export class NFTContractService {
  ...
  getContract(contractAddress: string, abi: any[] = ERC721_ABI): ethers.Contract {
    if (this.signer) {
  ...
  async mintNFT(
    ...
    try {
      ...
      const gasLimit = await contract.mint.estimateGas(
        recipientAddress,
        tokenURI,
        collection.royaltyBasisPoints || 750 // Default 7.5%
      );
      ...
      const tx = await contract.mint(
        recipientAddress,
        tokenURI,
        collection.royaltyBasisPoints || 750
      );
      ...
      const tokenData: InsertNftToken = {
        collectionId,
  ...
  async transferNFT(
    ...
    try {
      ...
      // Log transaction
      const txData: InsertNftTransaction = {
        tokenId,
        fromAddress,
        toAddress,
        type: 'transfer',
  ...
  async purchaseNFT(
    ...
    try {
      ...
      for (const royalty of royalties) {
        const royaltyData: InsertRoyaltyDistribution = {
  ...
  // Set royalty rules
  async setRoyaltyRules(
    tokenId: string,
    rules: InsertNftRoyaltyRule[]
  ): Promise<void> {
    try {
  ...
  // Get user's NFTs
  async getUserNFTs(userId: string): Promise<NftToken[]> {
    try {
  ...
  // Get collection NFTs
  async getCollectionNFTs(collectionId: string): Promise<NftToken[]> {
    try {
      const tokens = await db.select()
        .from(nftTokens)
