...
export class RoyaltyTrackerService {
  ...
  async trackRoyaltyPayment(data: InsertRoyaltyDistribution): Promise<RoyaltyDistribution> {
    try {
  ...
  async getUserRoyalties(
    ...
    try {
      ...
      if (startDate && endDate) {
        query = query.where(
          and(
            gte(royaltyDistributions.createdAt, startDate),
            lte(royaltyDistributions.createdAt, endDate)
          )
        );
      }
  ...
  async getCollectionRoyaltyAnalytics(collectionId: string): Promise<{
    totalVolume: {
      wei: string;
      usd: number;
    };
    ...
    recentDistributions: RoyaltyDistribution[];
  }> {
    try {
      ...
      const transactions = await db.select()
        ...
        .where(
          and(
            sql`${nftTransactions.tokenId} = ANY(${tokenIds})`,
            eq(nftTransactions.type, 'sale')
          )
        );
  ...
  async getPendingRoyalties(userId?: string): Promise<RoyaltyDistribution[]> {
    try {
      let query = db.select()
        .from(royaltyDistributions)
      ...
      if (userId) {
        query = query.where(
          and(
            eq(royaltyDistributions.status, 'pending'),
            eq(royaltyDistributions.recipientId, userId)
          )
        );
      }
  ...
  async processPendingRoyalties(): Promise<{
    processed: number;
    failed: number;
  }> {
    try {
  ...
  async getRoyaltyRules(
    ...
    try {
      ...
      if (tokenId) {
        query = query.where(
          and(
            eq(nftRoyaltyRules.tokenId, tokenId),
            eq(nftRoyaltyRules.isActive, true)
          )
        );
      } else if (collectionId) {
        query = query.where(
          and(
            eq(nftRoyaltyRules.collectionId, collectionId),
            eq(nftRoyaltyRules.isActive, true)
          )
        );
      }
  ...
  calculateDecayingRoyalty(
    baseBasisPoints: number,
    decayRate: number,
    transferCount: number,
    minBasisPoints: number = 100
  ): number {
  ...
  async getRoyaltySummaryDashboard(creatorId: string): Promise<{
    totalEarnings: {
      today: number;
      week: number;
      month: number;
    ...
    recentActivity: RoyaltyDistribution[];
    averageRoyaltyRate: number;
  }> {
    try {
      const now = new Date();
      ...
      for (const collection of collections) {
        ...
        const distributions = await db.select()
          ...
          .where(
            and(
              sql`${royaltyDistributions.tokenId} = ANY(${tokenIds})`,
              eq(royaltyDistributions.recipientId, creatorId),
              eq(royaltyDistributions.status, 'distributed')
            )
          );
      ...
      const rules = await db.select()
        ...
        .where(
          and(
            eq(nftRoyaltyRules.recipientId, creatorId),
            eq(nftRoyaltyRules.isActive, true)
          )
        );
