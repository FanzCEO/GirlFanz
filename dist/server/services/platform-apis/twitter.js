...
export class TwitterAPI {
    ...
    async postTweet(tweetData) {
        ...
        try {
            ...
            if (tweetData.mediaUrls && tweetData.mediaUrls.length > 0) {
                mediaIds = await Promise.all(tweetData.mediaUrls.map(url => this.uploadMedia(url)));
            }
            // Build tweet payload
    ...
    buildTweetPayload(tweetData, mediaIds) {
    ...
    // Helper methods
    buildTweetText(text, hashtags, mentions) {
        let tweetText = text;
    ...
    getAuthHeaders(method, url) {
        ...
        if (this.oauth && this.config.accessToken && this.config.accessTokenSecret) {
            const authData = this.oauth.authorize({ url, method }, {
