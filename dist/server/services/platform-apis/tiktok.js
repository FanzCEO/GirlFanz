export class TikTokAPI {
    ...
    async uploadVideoChunks(uploadUrl, videoUrl) {
    ...
    async getTrendingHashtags(region = 'US') {
    ...
    async calculateChunkCount(videoUrl) {
    ...
    // Get video list
    async getVideoList(cursor, maxCount = 20) {
        if (!this.config.accessToken) {
            throw new Error('TikTok access token not configured');
        }
